...
def convert_re_match_object_to_string(match):
  ...
  end = match.group().find(']')
  new_string = convert_title_to_sentence_case(match.group()[start: end+1], should_capitalize=False)
  return match.group()[:start] + new_string + match.group()[end+1:]
