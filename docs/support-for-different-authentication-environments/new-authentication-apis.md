...
To simplify the SAML and OIDC login workflow, we expose 2 new APIs `microstrategy.auth.samlLogin(serverUrl)` and `microstrategy.auth.oidcLogin(serverUrl)`. The details of these 2 APIs are as below:
