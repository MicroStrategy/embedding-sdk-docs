...
| props.getAuthToken | function  | The function for getting the login token. <br/>Similar to the getAuthToken in microstrategy.dossier.create. <br />In MicroStrategy 2021 Update 9, we only support auth token. You can get auth token with any auth mode. | true        |
